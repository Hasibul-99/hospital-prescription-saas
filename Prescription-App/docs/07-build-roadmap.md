...
- **Notification preferences (fixed)** — the follow-up reminder now honours doctor prefs correctly. `notify_followup_reminders` (default true) is the single gate for the patient reminder; `notify_email` (default false) is additive — when on, the doctor is CC'd on their patients' reminders. Previously `SendFollowUpReminderJob` AND-ed both flags, so the `notify_email` default of false silently killed reminders for any doctor with a profile row. `SendFollowUpRemindersCommand` now pre-filters opted-out doctors. The doctor Settings → Preferences UI toggles both (with tooltips).
- **Consolidated report PDF** — each role's reports page now has a "Full PDF" button that exports every section in one document (`?report=full` → `ReportExporter::pdfFullReport` → `resources/views/reports/full-pdf.blade.php`). Breakdowns render as DomPDF-friendly CSS bar charts (Recharts can't run in DomPDF); scalar sections (compliance, utilization, totals) render as key/value blocks. Per-card CSV/PDF exports still work as before.

### Still open (known gaps)

- No remaining items from the original 10-prompt spec. Net-new ideas (SMS, subscription payments, online booking portal, lab/investigation results, drug-class-aware allergy matching) are future work.
