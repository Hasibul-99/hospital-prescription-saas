...
export default function Index({ patients, filters }: Props) {
    ...
    function applyFilters() {
        router.get('/hospital/patients', getFilterParams(), { preserveState: true, replace: true });
    }

    function handleSort(column: string) {
        const dir = filters.sort_by === column && filters.sort_dir === 'asc' ? 'desc' : 'asc';
        router.get('/hospital/patients', { ...getFilterParams(), sort_by: column, sort_dir: dir }, { preserveState: true, replace: true });
    }
        ...
        <>
            <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <button onClick={exportCsv} className="rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700">Export CSV</button>
                    <Link href="/hospital/patients/create" className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">+ Register Patient</Link>
                </div>
            </div>
            ...
            <div className="mb-4 rounded-lg bg-white p-4 shadow">
                <div className="flex flex-wrap items-end gap-3">
                    ...
                    <button onClick={() => setShowMore(!showMore)} className="rounded bg-gray-100 px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-200">{showMore ? 'Less' : 'More Filters'}</button>
                </div>
            ...
            <div className="overflow-x-auto rounded-lg bg-white shadow">
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="border-b bg-gray-50 text-gray-600">
                            ...
                            <th className="cursor-pointer px-4 py-3" onClick={() => handleSort('age_years')}>Age/Gender{sortIcon('age_years')}</th>
                            <th className="cursor-pointer px-4 py-3" onClick={() => handleSort('phone')}>Phone{sortIcon('phone')}</th>
                    ...
                    <tbody>
                        {patients.data.length === 0 ? (
                            <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No patients found</td></tr>
                        ) : (
                            patients.data.map((p: any) => (
                                <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50">
                                    <td className="px-4 py-3 font-mono text-xs text-blue-600"><Link href={`/hospital/patients/${p.id}`}>{p.patient_uid}</Link></td>
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-2">
                                            <Link href={`/hospital/patients/${p.id}`} className="text-blue-600 hover:underline text-xs">View</Link>
                                            <Link href={`/hospital/patients/${p.id}/edit`} className="text-gray-600 hover:underline text-xs">Edit</Link>
                                        </div>
                                    </td>
                                </tr>
                            ))
                ...
                <div className="border-t px-4 py-3"><Pagination links={patients.links} /></div>
            </div>
        </>
    );
}
