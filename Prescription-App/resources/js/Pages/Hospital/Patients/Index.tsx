...
export default function Index({ patients, filters }: Props) {
    ...
    function handleSort(column: string) {
        ...
        router.get('/hospital/patients', { ...filters, sort_by: column, sort_dir: dir }, { preserveState: true, replace: true });
    }
    ...
    return (
        <>
            <div className="mb-4 flex items-center justify-between">
                ...
                <Link href="/hospital/patients/create" className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                    + Register Patient
                </Link>
            </div>
                            ...
                            patients.data.map((p: any) => (
                                <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50">
                                    <td className="px-4 py-3 font-mono text-xs text-blue-600">
                                        <Link href={`/hospital/patients/${p.id}`}>{p.patient_uid}</Link>
                                    </td>
