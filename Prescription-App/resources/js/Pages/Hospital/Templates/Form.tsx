...
export default function HospitalTemplateForm({
    template,
    complaint_masters,
    duration_presets,
    advice_suggestions,
    ...
    function save() {
        ...
        if (template) {
            router.put(`/hospital/templates/${template.id}`, payload as any, { onSuccess: onOk, onError: onFail });
        } else {
    ...
    return (
        <HospitalLayout>
            <Head title={template ? 'Edit Global Template' : 'New Global Template'} />
            <FlashMessage />

            <div className="mb-4 flex items-center justify-between">
                <Typography.Title level={4} className="!mb-0">
                    {template ? 'Edit Global Template' : 'New Global Template'}
                </Typography.Title>
                <Space>
                    <Button onClick={() => router.visit('/hospital/templates')}>Cancel</Button>
